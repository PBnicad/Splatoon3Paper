...
## 页面（触摸左右 1/6 翻页，底部标签跳页）

| 页 | 内容 |
|---|---|
| 涂地 | 一般比赛当前时段（高亮）+ 未来时段列表 |
| 真格 | 蛮颓系列/开放各自的当前时段卡（原生尺寸地图，不裁剪） |
| x | X比赛时段列表 |
| 活动 | 活动比赛（地图/描述）；放不下的活动以单行条目列出 |
| 祭典 | 独立页：祭典状态（阶段/时间/队伍/得票率/三色占地）或下次祭典、近期结算；祭典期间替换涂地/真格/x 页签，显示祭典常规/挑战时刻表；无任何祭典数据时标签自动隐藏 |
| 打工 | 当前班次（地图/王鲑/4 武器/倒计时）、未来班次、大型跑 ▲ 标记、团队打工竞赛 |
| 装备 | 鱿鱼限时装备（价格/下架时间/主能力）、打工月度奖励装备 |
